...
#Region "Metodos Privados"
    ...
    Private Sub _prCargarDetalleVenta(_numi As String)
        ...
        grdetalle.DataSource = L_fnDetalleMovimiento(_numi)
...
salirIf:
    ...
    Private Sub grdetalle_CellValueChanged(sender As Object, e As ColumnActionEventArgs) Handles grdetalle.CellValueChanged
        Dim pos, estado As Integer
        'LOTE
        Dim columna As String = "iclot"
        If e.Column.Index = grdetalle.RootTable.Columns(columna).Index Then
            If grdetalle.GetValue(columna).ToString() = String.Empty Then
                pr_ObtenerPosicionEstado(pos, estado, columna)
                CType(grdetalle.DataSource, DataTable).Rows(pos).Item(columna) = "20200101"
        ...
        If e.Column.Index = grdetalle.RootTable.Columns(columna).Index Then
            ...
            If Not IsDate(grdetalle.GetValue(columna)) Then
                pr_ObtenerPosicionEstado(pos, estado, columna)
                CType(grdetalle.DataSource, DataTable).Rows(pos).Item(columna) = "2020-01-01"
        ...
        If (e.Column.Index = grdetalle.RootTable.Columns(columna).Index) Then
            ...
            If (Not IsNumeric(grdetalle.GetValue(columna)) Or grdetalle.GetValue(columna).ToString = String.Empty) Then
                pr_ObtenerPosicionEstado(pos, estado, columna)
                CType(grdetalle.DataSource, DataTable).Rows(pos).Item(columna) = 1
            Else
                If (grdetalle.GetValue(columna) > 0) Then
                    pr_ObtenerPosicionEstado(pos, estado, columna)
                Else
                    pr_ObtenerPosicionEstado(pos, estado, columna)
                    CType(grdetalle.DataSource, DataTable).Rows(pos).Item(columna) = 1
    ...
    Private Sub pr_ObtenerPosicionEstado(ByRef pos As Integer, ByRef estado As Integer, columna As String)
        Dim lin As Integer = grdetalle.GetValue("icid")
        pos = -1
        _fnObtenerFilaDetalle(pos, lin)
        estado = CType(grdetalle.DataSource, DataTable).Rows(pos).Item("estado")
        If (estado = 1) Then
            CType(grdetalle.DataSource, DataTable).Rows(pos).Item("estado") = 2
        End If
    End Sub
    ...
    Private Sub grdetalle_CellEdited(sender As Object, e As ColumnActionEventArgs) Handles grdetalle.CellEdited
        ...
        Dim columna As String = "iccant"
        If (e.Column.Index = grdetalle.RootTable.Columns(columna).Index) Then
            If (Not IsNumeric(grdetalle.GetValue(columna)) Or grdetalle.GetValue(columna).ToString = String.Empty) Then

                grdetalle.SetValue(columna, 1)
            Else
                If (grdetalle.GetValue(columna) > 0) Then
                    Dim stock As Double = grdetalle.GetValue("stock")
                    If (grdetalle.GetValue(columna) > stock And cbConcepto.Value <> 1) Then
                        Dim lin As Integer = grdetalle.GetValue("icid")
                        Dim pos As Integer = -1
                        _fnObtenerFilaDetalle(pos, lin)
                        CType(grdetalle.DataSource, DataTable).Rows(pos).Item(columna) = stock
                        grdetalle.SetValue(columna, stock)
                        Dim img As Bitmap = New Bitmap(My.Resources.mensaje, 50, 50)
        ...
        If e.Column.Index = grdetalle.RootTable.Columns(columna).Index Then
            ...
            If grdetalle.GetValue(columna).ToString() = String.Empty Then
                grdetalle.SetValue(columna, "20200101")
            End If
        End If
        columna = "icfvenc"
        If e.Column.Index = grdetalle.RootTable.Columns(columna).Index Then
            If Not IsDate(grdetalle.GetValue(columna)) Then
                grdetalle.SetValue(columna, "2020-01-01")
            End If
        End If
    End Sub
    ...
    Private Sub btnEliminar_Click(sender As Object, e As EventArgs) Handles btnEliminar.Click
        ...
        Dim tMovimientoDetalle As DataTable = L_fnDetalleMovimiento(tbCodigo.Text)
        For i As Integer = 0 To tMovimientoDetalle.Rows.Count - 1 Step 1
            Dim _idProducto As Integer = tMovimientoDetalle.Rows(i).Item("iccprod")
            Dim _Lote As String = tMovimientoDetalle.Rows(i).Item("iclot")
            Dim _Fecha As Date = tMovimientoDetalle.Rows(i).Item("icfvenc")
            If L_fnVerificarExistenciaVenta(_idProducto, _Lote, _Fecha) Then
                resultado = True
            End If
        Next
        If resultado Then
            Dim img As Bitmap = New Bitmap(My.Resources.WARNING, 50, 50)
            ToastNotification.Show(Me, "No se puede eliminar el movimiento con código ".ToUpper + tbCodigo.Text + ", porque tiene transacciones relacionadas a la venta".ToUpper,
                                          img, 5000,
                                          eToastGlowColor.Green,
                                          eToastPosition.TopCenter)
            Exit Sub
